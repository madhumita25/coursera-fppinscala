...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val ans = right.filterAcc(p,left.filterAcc(p,acc))
    if (p(elem)) ans.incl(elem) else ans
  }

  def union(that: TweetSet): TweetSet = {
    //this.foreach(t => that.incl(t))
    //that
  ...
  def mostRetweeted: Tweet = {
    var best_tweet: Tweet = this.elem

    if (!left.isEmptySet) {
      val max_left: Tweet = left.mostRetweeted
      if (max_left.retweets > best_tweet.retweets) best_tweet = max_left
    }

    if (!right.isEmptySet) {
      val max_right: Tweet = right.mostRetweeted
      if (max_right.retweets > best_tweet.retweets) best_tweet = max_right
    }
    best_tweet
  }
